...
- The `search`, `count`, `get`, and `delete` tests for `DriftArticleDatastore` are passing.
- New API functions (`latestNews`, `popularNews`, `topNews`, `trendingNews`, `mostReadNews`, `highlight`) have been implemented in `DriftArticleDatastore`.

## What's Left to Build
- An example Flutter application demonstrating the package's usage.

## Current Status
The project is in a stable state, but some unit tests for the `ArticleDatastore` are currently failing.
...
- The unit tests for the newly implemented API functions (`latestNews`, `popularNews`, `topNews`, `trendingNews`, `mostReadNews`, `highlight`) are failing due to issues with mocking the `applyQueryParams` extension method.