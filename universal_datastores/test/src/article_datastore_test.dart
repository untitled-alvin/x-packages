...
void main() {
  setUpAll(() {
    registerFallbackValue(ArticleQueryParams());
    registerFallbackValue(ArticlesCompanion.insert(guid: '', ownerId: '', createdAt: Value(DateTime(2025))));
  ...
  group('DriftArticleDatastore', () {
    ...
    setUp(() {
      ...
      mockSelectStatement =
          MockSimpleSelectStatement<$ArticlesTable, ArticleData>();
      ...
      when(() => mockArticleDatabase.batch(any())).thenAnswer((invocation) async {
        final batchFunction =
            invocation.positionalArguments.first as Function(Batch);
        return batchFunction(mockBatch);
      });
      ...
      when(() => mockSelectStatement.get())
          .thenAnswer((_) async => []);
      when(() => mockSelectStatement.getSingleOrNull())
          .thenAnswer((_) async => null);
      
      when(() => mockBatch.insert(mockArticlesTable, any())).thenAnswer((_) async {});
      when(() => mockBatch.insertAll(mockArticlesTable, any())).thenAnswer((_) async {});
      when(() => mockBatch.insertAllOnConflictUpdate(mockSourcesTable, any())).thenAnswer((_) async {});
    });
    ...
    test('put with source inserts source', () async {
      ...
      verify(() => mockBatch.insertAllOnConflictUpdate(mockSourcesTable, any())).called(1);
      verify(() => mockBatch.insert(mockArticlesTable, any())).called(1);
    });
    ...
    test('count returns correct number of articles', () async {
      ...
      final result = await articleDatastore.count(ArticleQueryParams());
    ...
    test('search returns correct pagination data', () async {
      ...
      when(() => mockSelectStatement.get()).thenAnswer((_) async => data);
      when(() => mockSelectStatement.limit(any(), offset: any(named: 'offset')))
          .thenReturn(mockSelectStatement);

      final result = await articleDatastore.search(
        ArticleQueryParams(limit: 1, offset: 0),
      );
